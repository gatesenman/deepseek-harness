...
消费者获得一个确定性的、沙箱化的建模内核，可以安全地处理不可信文档。目前尚无编辑器组合——在 `apps/web` 之上构建桌面/网页工作台是自然的下一步，并将决定是否需要服务接缝。值类型约束（`pattern`/`min`/`max`/`enumValues`）作为定义被校验，但尚未在实例上强制执行；该强制执行属于同一个后续步骤。
